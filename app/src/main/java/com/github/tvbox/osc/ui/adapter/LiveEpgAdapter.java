...
//            timeview.setTextColor(getContext().getResources().getColor(R.color.color_theme));
            textview.setTextColor(((BaseActivity) getContext()).getThemeColor());
            timeview.setTextColor(((BaseActivity) getContext()).getThemeColor());
        } else {
            textview.setTextColor(getContext().getResources().getColor(R.color.color_FFFFFF));
            timeview.setTextColor(getContext().getResources().getColor(R.color.color_FFFFFF));
        }

        if (now.compareTo(value.startdateTime) >= 0 && now.compareTo(value.enddateTime) <= 0) {
            shiyi.setVisibility(View.VISIBLE);
            shiyi.setBackgroundColor(getContext().getResources().getColor(R.color.color_32364E));
            shiyi.setTextColor(getContext().getResources().getColor(R.color.color_FFFFFF));
            shiyi.setText("直播");
        } else if (now.compareTo(value.enddateTime) > 0 && source_include_back) {
            shiyi.setVisibility(View.VISIBLE);
            shiyi.setBackgroundColor(getContext().getResources().getColor(R.color.color_32364E_40));
            shiyi.setTextColor(getContext().getResources().getColor(R.color.color_FFFFFF));
            shiyi.setText("回看");
        } else if (now.compareTo(value.startdateTime) < 0 && source_include_back) {
            shiyi.setVisibility(View.VISIBLE);
            shiyi.setBackgroundColor(getContext().getResources().getColor(R.color.color_3D3D3D));
            shiyi.setTextColor(getContext().getResources().getColor(R.color.color_FFFFFF));
            shiyi.setText("预约");
        } else {
