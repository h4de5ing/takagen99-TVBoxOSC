...
public class JSEngine {
    ...
    public class JSThread {
        ...
        void initOkHttp() {
            ...
            jsContext.getGlobalObject().setProperty("joinUrl", args -> {
                URL url;
                String q = "";
                try {
            ...
            jsContext.getGlobalObject().setProperty("pdfh", args -> {
                try {
//                        LOG.i("pdfh----------------:"+args[1].toString().trim());
            ...
            jsContext.getGlobalObject().setProperty("pdfa", args -> {
                try {
//                        LOG.i("pdfa----------------:"+args[1].toString().trim());
            ...
            jsContext.getGlobalObject().setProperty("pd", args -> {
                try {
//                        LOG.i("pd----------------:"+args[2].toString().trim());
    ...
    public JSThread getJSThread() {
        ...
        if (thread == null) {
            ...
            JSModule.setModuleLoader(moduleName -> loadModule(moduleName));
    ...
    public void destroy() {
        for (String name : threads.keySet()) {
            JSThread jsThread = threads.get(name);
            if (jsThread != null && jsThread.thread != null) jsThread.thread.interrupt();
            if (jsThread.jsContext != null) jsThread.jsContext.destroyContext();
        }
        threads.clear();
    }
    ...
    public void stopAll() {
        ...
        for (String name : threads.keySet()) {
            JSThread jsThread = threads.get(name);
            if (jsThread.handler != null) jsThread.handler.removeCallbacksAndMessages(null);
        }
    }
